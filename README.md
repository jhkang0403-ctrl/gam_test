...
주제 12개 (5문항 또는 10문항). **모든 숫자는 공개 데이터에서 직접 계산한 값**입니다.
토요일 실습의 골격 저장소이고, 이대로 Vercel에 올리면 바로 돌아갑니다.

**12개가 모두 채워져 있습니다.** 실습에서는 자기가 맡은 한 주제를 지우고
...
```
   ...
   tools/fetch_all.py      공개 데이터 8곳에서 받기 (키·로그인 없음)
...
| | 주제 | 출처 |
|---|---|---|
| ⚽ | 한일전, 누가 더 이겼나 (10문항) | martj42/international_results (GitHub) |
| 📖 | 우리는 위키백과에서 뭘 찾아봤나 (10문항) | Wikimedia Pageviews API |
...
- **국가대표 A매치 기록**(GitHub 자원봉사 정리)의 한일전은 73경기지만, **대한축구협회 집계는
  80경기가 넘습니다.** 2025년 7월 EAFF E-1 결승 라운드도 빠져 있습니다
- **한국어 위키백과 월간 조회수 1위**는 2024·2025년 스물네 달 내내 '문화방송'(매달 100만 회 이상)
  입니다. 사람이 본 것인지 자동화된 접속인지 API 는 가려 주지 않습니다
