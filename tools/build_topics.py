...
topic(
    ...
    caveat=f"이 데이터는 유럽중앙은행(ECB) 고시 기준환율입니다. "
           f"시작일이 {FX[0][0]}라서 1997~98년 외환위기 구간이 통째로 "
           f"빠져 있습니다.")
...
topic(
    ...
    caveat=f"이 데이터에는 OWID_LIC(저소득국)·OWID_AFR(아프리카) 같은 "
           f"집계치 코드가 섞여 있어 국가 순위를 낼 때 걸러냈습니다. "
           f"최신 연도는 {l_latest}년으로, 그 이후 값은 아직 없습니다.")
